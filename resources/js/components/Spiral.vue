<template>
	<div>
		<!-- <div v-for="(random, randomKey) in randoms" :key="randomKey">
			
			<p class="tar" v-for="(breakdown, breakdownKey) in random.breakdowns" :key="breakdown.id">
		...
		<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMin slice" viewBox="0 0 728 400" v-for="(random, randomKey) in randoms" :key="randomKey">
		    ...
		    <textPath xlink:href="#s"> 
		    	{{ random.values }}				
		    	{{ 
					listOfBreakdowns(random.breakdowns)
				}} <!-- can't declare tag here to loop my random.braekdowns due to Spiral design. --> 
		    </textPath>
		    </text>  
		</svg> 
	</div>
...
export default {
	...
	methods: {
		...
		listOfBreakdowns(breakdowns)
		{ //had to do this cause SVG cant render multiple nested loop. 
			let lists = [];
